...
function Receipt() {
   ...
   const[ledgers, setLedgers]=useState([])
    function handleaddclick () {
            setLedgers([
                ...ledgers, {
                customer_name:inputList.customer_name,
...
//To get ledger list
    const data = async () => {
        ...
        const res = await fetch("http://localhost:8000/ledgerreceipt", {
            method: "GET",
            headers: {
 ...
 const [getcustomerdata, setCustomerdata] = useState([]);
    const customerData = async () => {
        ...
        const res = await fetch("http://localhost:8000/customerlist", {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
        ...
        if (res.status === 422 || ! customerdata) {
            ...
            console.log("got total customerdata ");
...
useEffect(() => { 
   data1()
    ...
    //get billno
    const getrecvoucherno = async () => {

        const res = await fetch("http://localhost:8000/getrecvoucherno", {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
    ...
    const savereceipt = async (e) => {
      ...
      console.log(e)
        ...
        const res = await fetch("http://localhost:8000/savereceipt", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
            voucher_no:newVoucherNo,
            customer_name:ledgers[0].customer_name,
        ...
        if (res.status === 422 || !receiptdata) {
            console.log(receiptdata);
            
            toast.success(receiptdata, {
              position: "top-right",
              autoClose: 3000,
            ...
            console.log("Receipt added "+{...inputList});
            ...
            <div className="p-8 w-full antialiased sans-serif">
                ...
                <div className="container mx-auto py-6 px-4">
                    <div className="flex justify-between">
                        <h2 className="text-2xl font-bold mb-6 pb-2 tracking-wider uppercase">Receipt Voucher</h2>
                        <div>
...
<p></p>
                            ...
                            <div className="flex  border-b py-2 items-start">
                                ...
                                <div className="flex-1 px-1">
                                    <select value={getcustomerdata.customer_name} name="customer_name"  className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                                        onChange={
                                            handleinputchange
                                    }>
...
<table class="table">
            <thead>
                <tr className="table-dark">
                    <th scope="col">#</th>
                    <th scope="col">Account/Received From/ Cr</th>
                    <th scope="col">Account/Received To/ Dr</th>
        ...
        <button onClick={savereceipt} className="bg-pink-500 text-white active:bg-pink-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150" type="button">
                                    Save
                                </button>
